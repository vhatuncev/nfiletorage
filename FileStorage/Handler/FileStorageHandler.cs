...
namespace FileStorage.Handler
{
    ...
    public class FileStorageHandler
    {
        ...
        private DataFileHeaderStruct _dataFileHeaderStruct = new DataFileHeaderStruct();
        ...
        private FileStorageHandler(string fileStorageName, VersionBehaviour versionBehaviour)
        {
            ...
            switch (versionBehaviour)
            {
                case VersionBehaviour.ValidateVersion:
                    ...
                    if (this._dataFileHeaderStruct.versionMajor < VersionMajor)
                    {
                        throw new NotSupportedException(string.Format("Version {2} conflict (file has major version number {0}, the code expects major version {1}). Please upgrade the container using the command tool; filestoragecmd upgrade old new", this._dataFileHeaderStruct.versionMajor, VersionMajor, GetVersion()));
                    }
                    else if (this._dataFileHeaderStruct.versionMajor == VersionMajor)
                    {
                        //
                        // check minor number
                        //
                        if (this._dataFileHeaderStruct.versionMinor < VersionMinor)
                        {
                            throw new NotSupportedException(string.Format("Version {2} conflict (file has minor version number {0}, the code expects minor version {1}). Please upgrade the container using the command tool; filestoragecmd upgrade old new", this._dataFileHeaderStruct.versionMinor, VersionMinor, GetVersion()));
                        }
                        else if (this._dataFileHeaderStruct.versionMinor == VersionMinor)
                        {
                            //
                            // ok, its supported
                            //
        ...
        private static void CreateNewFileStorage_IndexFile(string indexFilename)
        {
            using (var indexStream = File.Create(indexFilename))
            {
                ...
                var offset = OffsetFirstIndexBlock;
        ...
        private static void CreateNewFileStorage_DataFile(string dataFilename, List<FileStorageFeatureEnum> fileStorageFeatures)
        {
            ...
            var fileStorageFeaturesValue = FileStorageFeatureFactory.CreateFileStorageFeatureValue(fileStorageFeatures);
            ...
            using (var dataStream = File.Create(dataFilename))
            {
        ...
        public void RestoreIndexFile(AddFileBehaviour addFileBehaviour, StreamStateBehaviour indexStreamStateBehaviour, StreamStateBehaviour dataStreamStateBehaviour, ExposeProgressDelegate exposeProgressDelegate)
        {
            ...
            using (indexStream = FileStreamFactory.CreateFileStream(indexFilename, StreamStateBehaviour.OpenNewStreamForReadingAndWriting))
            {
                using (dataStream = FileStreamFactory.CreateFileStream(dataFilename, StreamStateBehaviour.OpenNewStreamForReading))
                {
                    ...
                    var offset = OffsetFirstDataBlock;

                    while (true)
                    {
                        var retrieveFileStruct = GetInterpretedDataStructUsingExistingStream(offset);
        ...
        {
            var serializedObject = DynamiteXmlLogic.Serialize(objectToStore);
        ...
        public void StoreBytesForExistingStreams(Guid dataIdentifier, byte[] fileData, ICustomMetaData customMetaData, AddFileBehaviour addFileBehaviour)
        {
            ...
            var supportsMetaData = SupportsFeature(FileStorageFeatureEnum.StoreMetaData);

            var dataOffset = dataStream.Length;
            ...
            if (supportsMetaData)
            {
                ...
                var metaDataString = DynamiteXmlLogic.Serialize(metaDataContainer);
                var metaDataBytes = ASCIIEncoding.UTF8.GetBytes(metaDataString);
        ...
        {
            var indexInGuid = 0;
            var currentOffsetInIndexFile = OffsetFirstIndexBlock;
        ...
        private Int64 GetOffsetInCurrentStream(Guid dataIdentifier, int currentIndexInGuid, Int64 currentOffset)
        {
            ...
            if (currentIndexInGuid < 15)
            {
                var nextOffset = currentIndexStruct.pointers[indexerInGuid];
        ...
        public AllocationTableStruct GetIndexStruct(Int64 currentOffset, StreamStateBehaviour indexStreamStateBehaviour)
        {
            ...
            //
            var binaryReader = new BinaryReader(indexStream);
            for (var elementIndex = 0; elementIndex < 256; elementIndex++)
            {
        ...
        public bool Exists(Guid dataIdentifier, StreamStateBehaviour indexStreamStateBehaviour)
        {
            ...
            var offset = GetOffset(dataIdentifier, indexStreamStateBehaviour);
            if (offset == 0)
            {
                //
                // does not exist
        ...
        public void ExportToFile(Guid dataIdentifier, string outputFile, ExportFileBehaviour exportFileBehaviour)
        {
            ...
            var fileData = GetFileByteData(dataIdentifier, StreamStateBehaviour.OpenNewStreamForReading, StreamStateBehaviour.OpenNewStreamForReading);
        ...
        public void ExportToOtherFileStorage(Guid dataIdentifier, FileStorageHandler targetFileStorageHandler, AddFileBehaviour addFileBehaviour, StreamStateBehaviour sourceIndexStreamStateBehaviour, StreamStateBehaviour sourceDataStreamStateBehaviour, StreamStateBehaviour targetIndexStreamStateBehaviour, StreamStateBehaviour targetDataStreamStateBehaviour)
        {
            ...
            var fileData = GetFileByteData(dataIdentifier, sourceIndexStreamStateBehaviour, sourceDataStreamStateBehaviour);
            var metaDataContainer = GetMetaDataContainer(dataIdentifier, sourceIndexStreamStateBehaviour, sourceDataStreamStateBehaviour);
        ...
        {
            var bytes = GetFileByteData(dataIdentifier, indexStreamStateBehaviour, dataStreamStateBehaviour);
        ...
        public MetaDataContainer GetMetaDataContainerForExistingStreams(Guid dataIdentifier)
        {
            ...
            var offset = GetOffset(dataIdentifier, StreamStateBehaviour.UseExistingStream);

            if (offset != 0)
            {
                var retrieveDataStruct = GetInterpretedDataStructUsingExistingStream(offset);
        ...
        public byte[] GetFileByteDataForExistingStreams(Guid dataIdentifier)
        {
            ...
            else
            {
                ...
                //
                throw new Exception(string.Format("DataIdentifier {0} does exist in filestorage {1}", dataIdentifier, this.fileStorageName));
            }
        ...
        {
            var indexStructs = GetIndexStruct(offset, indexStreamStateBehaviour);
            for (var currentByteValue = 0; currentByteValue <= 255; currentByteValue++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // continue recursion
                        //
                        var newOffset = indexStructs.pointers[currentByteValue];
            ...
            using (dataStream = new FileStream(dataFilename, FileMode.Open, FileAccess.Read))
            {
                ...
                else
                {
                    while (true)
                    {
                        ...
                        //
                        var retrieveFileStruct = GetInterpretedDataStructUsingExistingStream(offset);
                        var dataIdentifierAccordingToDataFile = retrieveFileStruct.DataIdentification;
                        //
